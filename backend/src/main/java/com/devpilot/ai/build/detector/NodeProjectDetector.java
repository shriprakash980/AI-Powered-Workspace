...
@Component
public class NodeProjectDetector implements ProjectDetector {

    @Override
    public boolean matches(List<ProjectFile> files) {
        return files.stream().anyMatch(f -> "package.json".equalsIgnoreCase(f.getName()));
    }

    @Override
    public ProjectDetectionResponse getDetails(List<ProjectFile> files) {
        return new ProjectDetectionResponse(
                List.of(ProjectType.NODEJS),
                ProjectType.NODEJS,
                "npm run build",
                "npm test",
                "npm start",
